...
/* global path is a shared variable*/
int *global_cost_path;
int *globals;
int T_ITER = 100;
...
/* @brief: SA algorithm for WSP problem, we implement two-level parallelism here
...
#pragma omp parallel num_threads(p) private(rand_position_1, rand_position_2)
	{
		// set two positions for swapping
		rand_position_1 = new int(0);
		rand_position_2 = new int(1);
		...
		// at first, multiple initial points
		...
		while (cnt < 300)
		{
			...
			int new_cost = random_swap_city_cost(cost_path, n_cities, dist, rand_position_1, rand_position_2, &rand_seeds[thread_id]);
			...
			else
			{
				...
				double rand_number = rand_r(&rand_seeds[thread_id]) / (RAND_MAX + 1.);
				if (rand_number < prob)
				{
					cost_path[0] = new_cost;
					cnt = 0;
		...
		memcpy(globals + thread_id * (n_cities + 1), cost_path, sizeof(int) * (n_cities + 1));
		delete[] cost_path;
		//	delete rand_position_1;
		//	delete rand_position_2;
	}

	// get the opmimal from globals
	int j = 0;
	int min = INT_MAX;
	for (int i = 0; i < (n_cities + 1) * p; i += (n_cities + 1))
	{
		if (globals[i] < min)
		{
			min = globals[i];
	...
	memcpy(global_cost_path, globals + j, sizeof(int) * (n_cities + 1));
	...
	while (temperature > 1e-5)
	{
		int cnt = 0;
		{
			...
			int *cost_path = new int[n_cities + 1];
			int thread_id = omp_get_thread_num();
			memcpy(cost_path, global_cost_path, sizeof(int) * (n_cities + 1));
...
#pragma omp atomic
		...
		int previous_min = global_cost_path[0];
		j = 0;
		min = INT_MAX;
		for (int i = 0; i < (n_cities + 1) * p; i += (n_cities + 1))
		{
		...
		memcpy(global_cost_path, globals + j, sizeof(int) * (n_cities + 1));
		//printf("when it ends, cnt is %d and temp is %f\n", cnt, temperature);
		// annealing step (i.e. reduce temperature)
		temperature *= 0.85;
		// get optimal and compare with previous one
	}
}
