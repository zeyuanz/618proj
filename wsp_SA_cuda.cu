...
cudaMemcpyAsync(dev_all_path + idx * (n_cities + 1), cost_path, sizeof(int)*(n_cities + 1), cudaMemcpyDeviceToDevice);
 }
...
void wsp_simulate_annealing_cuda(int *solution, int n_cities, int *dist, float *msec) 
{
	...
	int *global_min;
	int *dist_cu;
	cudaMalloc(&global_min, sizeof(int)*(n_cities + 1));
	...
	int index = 0;
	int min = dev_all_path[0];
	for(int i = 0; i < n_cities; i++) 
	{
		int local = dev_all_path[i * (n_cities + 1)];
		if(local < min)
		{
			index = i;
			min = local;
	...
	memcpy(global_min, dev_all_path + index * (n_cities + 1), sizeof(int)*(n_cities + 1));
	cudaEventRecord(stop);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(msec, start, stop);
	cudaMemcpy(solution, global_min, sizeof(int)*(n_cities + 1), cudaMemcpyDeviceToHost);
	cudaFree(&global_min);
